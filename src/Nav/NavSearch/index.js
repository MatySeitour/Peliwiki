import React from "react";
import "../NavSearch/NavSearch.css"

function NavSearch({ navState }) {
    const [searchState, setSearchActive] = React.useState(false);

    const searchActive = () => {
        if (!navState) {
            if (searchState === false) {
                setSearchActive(true)
            }
            else {
                setSearchActive(false)
            }
        }
    }

    return [
        <span onClick={searchActive} className="icon-search"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M500.3 443.7l-119.7-119.7c27.22-40.41 40.65-90.9 33.46-144.7C401.8 87.79 326.8 13.32 235.2 1.723C99.01-15.51-15.51 99.01 1.724 235.2c11.6 91.64 86.08 166.7 177.6 178.9c53.8 7.189 104.3-6.236 144.7-33.46l119.7 119.7c15.62 15.62 40.95 15.62 56.57 0C515.9 484.7 515.9 459.3 500.3 443.7zM79.1 208c0-70.58 57.42-128 128-128s128 57.42 128 128c0 70.58-57.42 128-128 128S79.1 278.6 79.1 208z" /></svg></span>,
        <div className={`${searchState ? `search-container` : `inactive`}`}>

            <span onClick={searchActive} className="back">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 512"><path d="M64 448c-8.188 0-16.38-3.125-22.62-9.375c-12.5-12.5-12.5-32.75 0-45.25L178.8 256L41.38 118.6c-12.5-12.5-12.5-32.75 0-45.25s32.75-12.5 45.25 0l160 160c12.5 12.5 12.5 32.75 0 45.25l-160 160C80.38 444.9 72.19 448 64 448z" /></svg>
            </span>

            <input type="text" placeholder="Search for movies, series, TV shows..." id=""></input>
        </div >
    ];
}

export { NavSearch }